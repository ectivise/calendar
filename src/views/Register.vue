<template>
  <v-container>
    <v-col cols="lg6 md6 sm12 xs12">
      <v-form>
        <h1>Register</h1>
        <v-text-field label="Phone Number" v-model="logininfo.phonenumber" required :rules="phonerules"></v-text-field>
        <v-text-field label="Password" v-model="logininfo.password" required :rules="[v => !!v || 'item is required for Login']" type="password"></v-text-field>

        <!-- login with OTP -->
        <v-dialog v-model="dialog" persistent max-width="600px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" v-bind="attrs" v-on="on" class="ma-3" @click="handlesignup()" :disabled="regdisabled">Register</v-btn>
          </template>
          <v-card>
            <v-card-title>
